...
router.get("/how-make-a-css-print-ready-stylesheet", function(req, res, next) {
	res.render("how-to-print-css", {
		title: "چطور css رو برای پرینت آماده کنیم",
	});
});
	...
	async function(req, res, next) {
		...
		var options = {
			...
			header: {
				height: "35mm",
				contents:
					'<table class="header-table" style="text-align:center;"><tr><td style="font-weight: bold;text-align:right;">' +
					filename +
			...
			footer: {
				...
				contents: {
					...
					2: "صفحه‌ی دوم",
					default:
						'<div style="color: #333; font-size: 9px;border-top:1px solid #eee;">صفحه‌ی {{page}} از {{pages}}</div>',
				},
			},
		};

		// Path to save a temporary rendered file
		var htmlFilename = path.join(
			__dirname,
			"../public/downloads/" + filename + ".html",
		);

		// Path to save the pdf document from generated html file
		var pdfFilename = path.join(
			__dirname,
			"../public/downloads/" + filename + ".pdf",
		);

		// Path to the theme stylesheet
		var themeFilename = path.join(
			__dirname,
			"../public/stylesheets/prints/theme-" + theme + ".css",
		);
		...
		await Twig.renderFile(
			__dirname + "/../views/print.html.twig",
			{
				content: html,
				title: filename,
				author: author,
			...
			(err, twigResult) => {
				...
				pdf.create(tempHtmlFile, options).toFile(pdfFilename, function(
					err,
					renderedPdf,
				) {
					if (err)
